...
class DetectionCubit extends Cubit<DetectionState> {
  ...
  Future<void> startDetection() async {
    ...
    try {
      ...
      _detectionSubscription =
        ...
        debugPrint(
            'DetectionCubit: Received detection result - Level: ${result.level}, Confidence: ${result.confidence}');
        emit(DetectionResultUpdated(result));
      });

      if (_detectionSubscription == null) {
        debugPrint(
            'DetectionCubit: WARNING - Failed to establish stream subscription');
  ...
  Future<void> stopDetection() async {
    ...
    try {
      ...
      // CRITICAL FIX: Only emit DetectionStopped state
      // Do NOT emit DetectionResultUpdated with reset result
      // This prevents button toggling when detection stops
  ...
  Future<void> processCameraFrame(CameraImage cameraImage) async {
    try {
      ...
      await _detectionService.processCameraImage(
          cameraImage, 0); // 0 for default orientation
    } catch (e) {
